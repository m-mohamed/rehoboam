...
Releases are fully automated via [release-plz](https://release-plz.ieni.dev/) + [cargo-dist](https://opensource.axo.dev/cargo-dist/).

**For contributors:** Just merge your PR to main. That's it.

**What happens automatically:**
1. **release-plz** analyzes commits and creates a Release PR with:
   - Version bump in `Cargo.toml` (based on conventional commits)
   - Updated `CHANGELOG.md` (generated by git-cliff)
2. A maintainer reviews and merges the Release PR
3. **release-plz** creates the git tag and publishes to crates.io
4. **cargo-dist** builds binaries for 4 platforms and creates GitHub Release

```
Feature PR → Merge → release-plz creates Release PR → Merge → Ship
```

No manual version bumping, changelog editing, or tagging required.

### Changelog Format

Changelogs are auto-generated in [Keep a Changelog](https://keepachangelog.com/) format:
...
To control changelog entries, use [Conventional Commits](https://www.conventionalcommits.org/):
- `feat:` → Added section
- `fix:` → Fixed section
- `perf:`, `refactor:`, `chore:` → Changed section
